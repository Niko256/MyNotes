...
То есть атомарная операция переводит систему из одного согласованного состояния в другое состояние, без возможности наблюдения или вмешательства со стороны других операций в промежуточные подсостояния, которые могут возникнуть во время её выполнения.
...
Мы установили, что **атомарность** - это критически важное свойство неделимости операции в среде параллельных вычислений, защищающее систему от хаоса состояний гонки. Но как же воспользоваться этим свойством на практике? Как потребовать, чтобы конкретная операция, скажем, увеличение счетчика или обновление указателя, действительно была выполнена атомарно?

 В этом помогают **атомарные типы данных. Атомарные типы** - это типы, для которых определены **атомарные операции**, предоставляемые на уровне языка и аппаратном уровне. Именно через атомарные типы и операции над ними, разработчики встречаются с проявлением/воплощением модели памяти языка программирования.
 
  И так, атомики (атомарные типы) инкапсулируют низкоуровневые аппаратные или программные механизмы, необходимые для обеспечения атомарности, и представляют их в стандартизированном виде. Например, в `C++` стандартная библиотека предоставляет шаблонный тип `std::atomic<T>`, который позволяет создавать объекты типа `T`, операции над которыми будут атомарными.
...
В `C++` ей соответствуют `std::atomic::compare_exchange_strong` и `std::atomic::compare_exchange_weak`. Такое "обилие" из `strong` и `weak` версий обусловлено тем, что на архитектурах процессоров, отличных от `x86-64` операция `CAS` выполняется иначе. И например на `ARM` `CAS` операция может вернуть `false`, несмотря на то, что в реальности значения операндов совпадали. Для подобных архитектур и существует `compare_exchange_strong`, которая в реализации на уровне ассемблера повторяет операцию сравнения, если произошла так называемая `spurious failure`.
